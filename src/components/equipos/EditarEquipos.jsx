...
const EditarEquipo = () => {
    ...
    const buscarEquipoPorCodificacion = async () => {
        try {
            const response = await EquiposServices.obtenerPorCodificacion(codificacion);
            ...
            if (response.data) {
                ...
                if (response.data.imagenRuta) {
                    setImagenPreview(
                        `${import.meta.env.VITE_API_URL}/equipos/${response.data.imagenRuta}`
                    );
                }

                setModoEdicion(true);
            } else {
                toast.warn("Equipo no encontrado con esa codificación");
            }
        } catch (error) {
            toast.error("Error al buscar equipo por codificación");
        }
    };
