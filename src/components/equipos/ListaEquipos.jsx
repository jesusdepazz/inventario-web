...
const ListaEquipos = () => {
    ...
    const [campoBusqueda, setCampoBusqueda] = useState("codificacion");
    ...
    const resultadosFiltrados = equipos.filter((equipo) => {
        if (!valorBusqueda.trim()) return true;

        let valorCampo = "";

        if (campoBusqueda === "asignaciones") {
            valorCampo = equipo.asignaciones
                ?.map(a => `${a.codigoEmpleado || ""} ${a.nombreEmpleado || ""} ${a.puesto || ""}`)
                .join(" ") || "";
        } else {
            valorCampo = equipo[campoBusqueda] !== null && equipo[campoBusqueda] !== undefined
                ? equipo[campoBusqueda].toString()
                : "";
        }
        console.log("Campo:", campoBusqueda, "Valor en equipo:", valorCampo);

        return valorCampo.toLowerCase().includes(valorBusqueda.toLowerCase());
    });
            ...
            <div className="overflow-x-auto w-full bg-white shadow-md rounded-xl p-6">
                <div className="mb-4 flex items-center gap-4">
                    ...
                    >
                        ...
                        <option value="asignaciones">Asignado a</option>
                    </select>
                ...
                <table
                    className={`w-full text-sm text-left border border-gray-200 ${valorBusqueda.trim() === "" ? "min-w-[4000px]" : "max-w-lg mx-0"
                        }`}
                >
                    <thead className="bg-blue-900">
                        {valorBusqueda.trim() === "" ? (
                            ...
                            <tr className="text-center bg-blue-900 text-white">
                                <th className="px-4 py-2 border">#</th>
                                <th className="px-4 py-2 border">{campoBusqueda}</th>
                            </tr>
                        )}
                    </thead>
                    ...
                    <tbody>
                        {resultadosFiltrados.length > 0 ? (
                                ...
                                <tr key={equipo.id} className="text-center">
                                    {valorBusqueda.trim() === "" ? (
                                        ...
                                        <>
                                            <td className="px-4 py-2 border">{index + 1}</td>
                                            <td className="px-4 py-2 border">
                                                {campoBusqueda === "asignaciones"
                                                    ? equipo.asignaciones?.map(a => `${a.codigoEmpleado} ${a.nombreEmpleado}`).join(", ")
                                                    : equipo[campoBusqueda]}
                                            </td>
                                        </>
                                    )}
                                </tr>
                            ...
                            <tr>
                                <td colSpan={valorBusqueda.trim() === "" ? "26" : "2"} className="text-center p-4 text-gray-600">
